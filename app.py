...
# ===============================
# HELPER: DOWNLOAD EXCEL DENGAN HIGHLIGHT WARNA
# ===============================
def to_excel_with_highlight(df, missing_mask=None, placeholder_mask=None, dup_row_mask=None, sheet_name="Data"):
    """
    Konversi dataframe ke file Excel (.xlsx) dengan cell background color:
    - merah  : kosong asli (NaN)
    - biru   : placeholder (mis. '-', 'N/A')
    - kuning : baris duplikat kolom kunci
    Pewarnaan diterapkan ke SEMUA baris (tidak dibatasi jumlah sel),
    jadi tetap muncul walau datanya banyak.
    """
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        fill_missing = PatternFill(start_color="FFFEE2E2", end_color="FFFEE2E2", fill_type="solid")
        fill_placeholder = PatternFill(start_color="FFDBEAFE", end_color="FFDBEAFE", fill_type="solid")
        fill_dup_row = PatternFill(start_color="FFFEF9C3", end_color="FFFEF9C3", fill_type="solid")

        columns = list(df.columns)
        n_cols = len(columns)

        for row_idx, idx in enumerate(df.index, start=2):  # baris 1 = header
            is_dup_row = (
                dup_row_mask is not None
                and idx in dup_row_mask.index
                and bool(dup_row_mask.loc[idx])
            )
            if is_dup_row:
                for col_idx in range(1, n_cols + 1):
                    ws.cell(row=row_idx, column=col_idx).fill = fill_dup_row

            for col_idx, col in enumerate(columns, start=1):
                is_placeholder = (
                    placeholder_mask is not None
                    and col in placeholder_mask.columns
                    and bool(placeholder_mask.at[idx, col])
                )
                is_missing = (
                    missing_mask is not None
                    and col in missing_mask.columns
                    and bool(missing_mask.at[idx, col])
                )
                if is_placeholder:
                    ws.cell(row=row_idx, column=col_idx).fill = fill_placeholder
                elif is_missing:
                    ws.cell(row=row_idx, column=col_idx).fill = fill_missing
...
if st.session_state.page == "Upload & Preview":
    ...
    # Baris "tidak ideal" = jumlah polos baris kena null/placeholder + baris duplikat persis.
    # Kalau ada baris yang kena DUA-DUANYA sekaligus, baris itu ikut kehitung 2x di sini
    # (sengaja, biar kelihatan skala masalahnya secara keseluruhan, bukan baris unik).
    rows_not_ideal_preview = rows_with_missing_preview + full_row_dup_preview
...
# =========================================================
    ...
    with header_right:
        dup_row_mask_for_excel = (
            (df_clean["catatan_duplikat"] != "") if "catatan_duplikat" in df_clean.columns else None
        )
        excel_bytes = to_excel_with_highlight(
            display_df,
            missing_mask=missing_style_mask,
            placeholder_mask=placeholder_style_mask,
            dup_row_mask=dup_row_mask_for_excel,
...
# =========================================================
    ...
    if display_df is not None and clean_rows_mask is not None:
        ...
        if clean_rows_count > 0:
            ...
            with info_right:
                clean_excel_bytes = to_excel_with_highlight(
                    clean_only_df,
                    sheet_name="Data Bersih",
                )
                st.download_button(
                    label="⬇️ Download Data Bersih",
                    data=clean_excel_bytes,
